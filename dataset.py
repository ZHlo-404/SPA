...
class CompositionDataset(tdata.Dataset):
    def __init__(
        ...
        self.cfg = cfg
        self.split = split
        self.open_world = open_world    #useless
        self.split_files = cfg.DATASET.split_files_loc  #useless
        ...
        self.all_attrs = self.train_attrs + self.val_attrs + self.test_attrs    #和上面self.attrs, self.objs一样，只是顺序不一样
        ...
        self.extra_objs = sorted(list(set(self.extra_objs) - set(self.train_objs)))
        self.extra_attrs = sorted(list(set(self.extra_attrs) - set(self.train_attrs)))
        self.extra_pairs = sorted(list(set(self.extra_pairs) - set(self.train_pairs)))
        print('New_attrs:',len(self.extra_attrs),' New objs:',len(self.extra_objs),' New pairs:',len(self.extra_pairs))
        

        # self.unique_pairs =  self.pairs + self.extra_pairs      #12695
        self.unique_pairs =  self.pairs + sorted(list(set(self.extra_pairs) - set(self.pairs)))
        self.unique_pair2idx = {pair: idx for idx, pair in enumerate(self.unique_pairs)}    #12491
        ...
        # self.unique_attrs =  self.all_attrs + self.extra_attrs    # 553
        self.unique_attrs =  self.all_attrs + sorted(list(set(self.extra_attrs) - set(self.all_attrs)))   
        self.unique_attr2idx = {attr: idx for idx, attr in enumerate(self.unique_attrs)}    #533
        
        # self.unique_objs =  self.all_objs + self.extra_objs     #1116
        self.unique_objs =  self.all_objs + sorted(list(set(self.extra_objs) - set(self.all_objs)))
        self.unique_obj2idx = {obj: idx for idx, obj in enumerate(self.unique_objs)}    #1065

        self.train_pairs_extra =  self.train_pairs + self.extra_pairs   #11688
        self.train_extra_pair2idx = {pair: idx for idx, pair in enumerate(self.train_pairs_extra)}     #  11688

        self.train_attrs_extra =  self.train_attrs + self.extra_attrs   #522
        self.train_extra_attr2idx = {attr: idx for idx, attr in enumerate(self.train_attrs_extra)}      #522

        self.train_objs_extra =  self.train_objs + self.extra_objs  #1053
        self.train_extra_obj2idx = {obj: idx for idx, obj in enumerate(self.train_objs_extra)}  #1053
        ...
        #load unseen pairs
        #len(self.unseen_pairs)=772 len(self.unseen_pair2idx)=772
    ...
    def parse_split(self):
        ...
        print('train:',len(tr_pairs),'test:',len(ts_pairs),'val:',len(va_pairs))
        ...
        only_test_attrs = sorted(list(set(ts_attrs) - set(tr_attrs)))     #ts or tr? but no Influence
        only_test_objs = sorted(list(set(ts_objs) - set(tr_objs)))

        only_val_attrs = sorted(list(set(va_attrs) - (set(tr_attrs + te_attrs))))
        only_val_objs = sorted(list(set(va_objs) - set(tr_objs + te_objs)))
        # pdb.set_trace()
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'train':
            ...
            L, At, Ob, L1, At1, Ob1= self.sample_neighbor_features(attr,obj)
            data['lbl1'], data['lbl1_L'] = L[0], L1[0]
            data['lbl2'], data['lbl2_L'] = L[1], L1[1]
            data['lbl3'], data['lbl3_L'] = L[2], L1[2]
            data['lbl4'], data['lbl4_L'] = L[3], L1[3]
            data['lbl5'], data['lbl5_L'] = L[4], L1[4]

            data['extra_lbl'] = L

            data['at1'], data['at1_L'] = At[0], At1[0]
            data['at2'], data['at2_L'] = At[1], At1[1]
            data['at3'], data['at3_L'] = At[2], At1[2]
            data['at4'], data['at4_L'] = At[3], At1[3]
            data['at5'], data['at5_L'] = At[4], At1[4]

            data['ob1'], data['ob1_L'] = Ob[0], Ob1[0]
            data['ob2'], data['ob2_L'] = Ob[1], Ob1[1]
            data['ob3'], data['ob3_L'] = Ob[2], Ob1[2]
            data['ob4'], data['ob4_L'] = Ob[3], Ob1[3]
            data['ob5'], data['ob5_L'] = Ob[4], Ob1[4]
    ...
    def sample_neighbor_features(self,attr,obj):
        ...
        L1 = [self.train_extra_pair2idx[k] for k in new]    #use for label
        L = [self.unique_pair2idx[k] for k in new]         #use for text embedding
        #### attrs neighbors
        neigh_attr = list(self.seen_attr_neighbors[attr].keys())
        new_attr = random.sample(neigh_attr,5)
        AT1 = [self.train_extra_attr2idx[k] for k in new_attr]      #use for label
        AT = [self.unique_attr2idx[k] for k in new_attr]            #use for text embedding

        #### objs neighbors
        neigh_obj = list(self.seen_obj_neighbors[obj].keys())
        new_obj = random.sample(neigh_obj,5)
        OB1 = [self.train_extra_obj2idx[k] for k in new_obj]        #use for label
        OB = [self.unique_obj2idx[k] for k in new_obj]              #use for text embedding

        return L, AT, OB, L1, AT1, OB1
