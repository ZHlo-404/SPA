...
class Evaluator:
    ...
    def generate_predictions(self, scores, obj_truth, bias = 0.0, topk = 5): # (Batch, #pairs)
        ...
        results.update({'open': get_pred_from_scores(scores, topk)})    #attr pred, obj pred
        results.update({'unbiased_open': get_pred_from_scores(orig_scores, topk)})  #attr pred, obj pred
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
        ...
        correct_scores = scores[torch.arange(scores.shape[0]), pair_truth][unseen_ind]  #correct scores:所有未见组合样本中对应truth pair的得分
      
        max_seen_scores = predictions['scores'][unseen_ind][:, self.seen_mask].topk(topk, dim=1)[0][:, topk - 1]    #max_seen_scores:所有未见组合样本中对应seen pair的最大得分
        ...
        sa_so_list, ua_uo_list, sa_uo_list, ua_so_list, sa_so_u_list = [], [], [], [], []
        for bias in biaslist:
            ...
            scores_1 = base_scores.clone()
            mask_1 = self.seen_mask.repeat(scores_1.shape[0],1)
            scores_1[~mask_1] += bias
            mask_1 = self.closed_mask.repeat(scores_1.shape[0], 1)
            closed_scores_1 = scores_1.clone()
            closed_scores_1[~mask_1] = -1e10
            scores_1 = closed_scores_1
            sa_so_acc = float(self.accuracy(scores_1[sa_so].detach().cpu(), pair_truth[sa_so],[1])[0])
            ua_so_acc = float(self.accuracy(scores_1[ua_so].detach().cpu(), pair_truth[ua_so],[1])[0])
            sa_uo_acc = float(self.accuracy(scores_1[sa_uo].detach().cpu(), pair_truth[sa_uo],[1])[0])
            ua_uo_acc = float(self.accuracy(scores_1[ua_uo].detach().cpu(), pair_truth[ua_uo],[1])[0]) 
            sa_so_u_acc = float(self.accuracy(scores_1[sa_so_u].detach().cpu(), pair_truth[sa_so_u],[1])[0])

            sa_so_list.append(sa_so_acc)
            ua_so_list.append(ua_so_acc)
            sa_uo_list.append(sa_uo_acc)
            ua_uo_list.append(ua_uo_acc)
            sa_so_u_list.append(sa_so_u_acc) 
        ...
        stats['mean_acc'] = np.mean([sa_so_acc[0].tolist()[0], sa_so_u_acc[0].tolist()[0], sa_uo_acc[0].tolist()[0], ua_so_acc[0].tolist()[0], ua_uo_acc[0].tolist()[0]])
