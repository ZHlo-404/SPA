...
class GradientCorrectionMLP(nn.Module):
    def __init__(self, input_dim=512, hidden_dim=512, dropout=0.2):
        super().__init__()
    ...
    def _init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                # 使用 Kaiming 均匀初始化
                init.kaiming_uniform_(m.weight, nonlinearity='relu')  # 使用 ReLU 激活函数
    ...
    def ortho_loss(self):
        ...
        for layer in [self.fc1, self.fc2]:
            loss += F.mse_loss(torch.matmul(layer.weight, layer.weight.T), torch.eye(layer.weight.shape[0]).to(layer.weight.device))
        return loss



class DeepGradientCorrectionMLP(nn.Module):
    def __init__(self, input_dim=512, hidden_dim=1024, num_layers=6, dropout=0.3):
        """
        Args:
            input_dim: 输入维度（需与梯度维度相同）
            hidden_dim: 隐层维度（建议大于输入维度以提升容量）
            num_layers: 总层数（每个残差块含2个线性层）
            dropout: 分层Dropout概率
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 输出投影 + 全局残差
        x = self.output_fc(x)
        return x + identity  # 输入到输出的残差连接

    def ortho_loss(self):
        """分层正交正则化"""
        loss = 0.0
        for name, param in self.named_parameters():
            if 'weight' in name and ('res_blocks' in name or 'input_fc' in name or 'output_fc' in name):
                # 计算权重矩阵的正交性损失
                w = param.view(param.size(0), -1)
                wt_w = torch.mm(w.t(), w)
                I = torch.eye(w.size(1), device=w.device)
                loss += F.mse_loss(wt_w, I, reduction='mean')
        return loss / len(list(self.named_parameters()))  # 平均损失
