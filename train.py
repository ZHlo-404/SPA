...
def train(epoch, model, optimizer, trainloader, logger, device, cfg):
    ...
    for idx, batch in enumerate(tqdm(trainloader)):
        ...
        # ###consistency loss
        optimizer.zero_grad()
        train_attr_len = len(model.module.dset.train_attrs)
        train_obj_len = len(model.module.dset.train_objs)
        all_attr_len = len(model.module.dset.all_attrs)
        all_obj_len = len(model.module.dset.all_objs)

        seen_attr, seen_obj = model.module.soft_embedding[:train_attr_len], model.module.soft_embedding[model.module.offset:model.module.offset+train_obj_len]
        init_seen_attr, init_seen_obj = model.module.init_token_embedding1[:train_attr_len].clone().detach(), \
                                        model.module.init_token_embedding1[model.module.offset:model.module.offset+train_obj_len].clone().detach()
        
        ## text consistency loss
        init_seen_attr_text = model.module.token2text(init_seen_attr, 'attrs')
        init_seen_obj_text = model.module.token2text(init_seen_obj, 'objs')
        ...
        seen_attr_text = model.module.token2text(seen_attr, 'attrs')
        seen_obj_text = model.module.token2text(seen_obj, 'objs')
        ...
        T=0.1
        loss_consistency = F.kl_div(F.log_softmax(update_sim_attr_text_topk/T, dim=1), F.softmax(model.module.init_sim_attr_text_topk/T, dim=1)) + F.kl_div(F.log_softmax(update_sim_obj_text_topk/T, dim=1), F.softmax(model.module.init_sim_obj_text_topk/T, dim=1))
...
def main_worker(gpu, cfg):
    ...
    # if gpu == 0:
        # stats_test = validate_ge(epoch, m, testloader, evaluator_test_ge, device, optimizer,'test')
