...
from models.troika.troika_ov import Troika
from dataset import CompositionDataset
import evaluator as evaluator_ge
from tqdm import tqdm
import matplotlib.pyplot as plt
import seaborn as sns
from models.troika.loss import loss_calu
...
def train(epoch, model, optimizer, trainloader, logger, device, cfg):
    ...
    for idx, batch in enumerate(tqdm(trainloader)):
        ...
        update_sim_attr_text_topk = torch.gather(update_sim_attr_text, 1, m.init_sim_attr_text_topk_indices)
        update_sim_obj_text_topk = torch.gather(update_sim_obj_text, 1, m.init_sim_obj_text_topk_indices)

        T=0.1
        loss_consistency = F.kl_div(F.log_softmax(update_sim_attr_text_topk/T, dim=1), F.softmax(m.init_sim_attr_text_topk/T, dim=1)) + F.kl_div(F.log_softmax(update_sim_obj_text_topk/T, dim=1), F.softmax(m.init_sim_obj_text_topk/T, dim=1))
        ...
        # T=0.1
        # loss_consistency = F.kl_div(F.log_softmax(update_sim_attr_text_topk/T, dim=1), F.softmax(model.init_sim_attr_text_topk/T, dim=1)) + F.kl_div(F.log_softmax(update_sim_obj_text_topk/T, dim=1), F.softmax(modelinit_sim_obj_text_topk/T, dim=1))
