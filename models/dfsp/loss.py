...
def loss_calu(predict, target):
    loss_fn = CrossEntropyLoss()
    batch_img, batch_attr, batch_obj, batch_target = target['img'], target['attr'], target['obj'], target['pair']
    ...
    loss = loss_logit_df + 0.01 * (loss_att + loss_obj) + 0.1 * loss_logit_sp
    return loss