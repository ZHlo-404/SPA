...
def initialize_wordembedding_matrix(name, vocab, cfg):
    ...
    for wordemb in wordembs:
        ...
        elif 'CLIP' in wordemb:
            wordemb_ = load_CLIP_embeddings(wordemb,vocab,cfg)
        if result is None:
            result = wordemb_
        else:
...
def load_CLIP_embeddings(wordemb,vocab,cfg):
    '''
    Inputs
        emb_file: Text file with word embedding pairs e.g. Glove, Processed in lower case.
        vocab: List of words
    ...
    if 'CLIP' in wordemb:
        path_emb = cfg.DATASET.split_files_loc
        # print("1")
        # m = torch.load(path_emb+'/MY_CLIP_ALL_feats_'+cfg.DATASET.dset_name+'.pt')
        # m1 = torch.load(path_emb+'/MY_CLIP_Neighbor_bert_text_features_prompt1_'+cfg.DATASET.dset_name+'.pt')
        m = torch.load(path_emb+'/ALL_feats_'+cfg.DATASET.dset_name+'.pt')
        m1 = torch.load(path_emb+'/Neighbor_bert_text_features_prompt1_'+cfg.DATASET.dset_name+'.pt')
        
        # # #tokenize embedding
        # m = tokenize_embedding(m)
        # m1 = tokenize_embedding(m1)
    ...
    # custom_map2 = {'abandoned':'left'}
    embeds = []
    # print(f"model key length:{len(model.keys())}")
    # print(f"model2 key length:{len(model2.keys())}")
    for k in vocab:
        if k in model.keys():
            emb = model[k].cpu()
        else: 
        ...
        embeds.append(emb)
    embeds = torch.stack(embeds)
    print('CLIP Embeddings loaded, total embeddings: {}'.format(embeds.size()))
    return embeds
